...
## Contract Test Cases (Foundry) — 46/46 pass

### IntentDOT.t.sol — 27 tests
| # | Test | Expected | Status |
|---|------|----------|--------|
| 1 | `test_swap_DOT_USDT()` | Correct output amount, reserves updated | PASS |
| 2 | `test_swap_insufficient_balance()` | Revert with ERC20InsufficientBalance (OZ custom error) | PASS |
...
| 18 | `test_pause_blocks_swap()` | Swap reverts with EnforcedPause when paused (OZ Pausable) | PASS |
| 19 | `test_pause_blocks_transfer()` | Transfer reverts with EnforcedPause when paused | PASS |
| 20 | `test_unpause_resumes_swap()` | Swap works after unpause | PASS |
| 21 | `test_non_owner_cannot_pause()` | Revert OwnableUnauthorizedAccount for non-owner | PASS |
| 22 | `test_factory_can_whitelist()` | Factory role can whitelist | PASS |
| 23 | `test_non_owner_cannot_set_factory()` | Revert OwnableUnauthorizedAccount (OZ Ownable) | PASS |
| 24 | `test_swapWithPermit_happy_path()` | ERC20Permit: permit→swap in one tx, no prior approve | PASS |
| 25 | `test_swapWithPermit_expired_deadline()` | ERC20Permit: expired deadline reverts | PASS |
| 26 | `test_swapWithPermit_wrong_signer()` | ERC20Permit: invalid signature reverts | PASS |
| 27 | `test_swapWithPermit_replay_reverts()` | ERC20Permit: nonce consumed, replay blocked | PASS |

### TokenFactory.t.sol — 8 tests
| # | Test | Expected | Status |
|---|------|----------|--------|
...
| 8 | `test_createToken_reverts_no_role()` | Revert AccessControlUnauthorizedAccount (OZ AccessControl) | PASS |
...
| 3 | `test_setRiskEngine_only_owner()` | Revert OwnableUnauthorizedAccount (OZ Ownable) | PASS |
...
- [x] All Foundry tests pass (`forge test`) — 46/46
