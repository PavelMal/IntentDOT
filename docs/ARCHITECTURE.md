...
- **Tech:** Solidity 0.8.24, Foundry, OpenZeppelin 5.x (Ownable, ReentrancyGuard, Pausable, SafeERC20, IERC20Permit)
- **Functions:** executeSwap, executeSwapWithPermit, executeTransfer, previewSwap, whitelistToken, setFactory, setRiskEngine, pause, unpause
- **Permit Flow:** `executeSwapWithPermit()` accepts EIP-2612 permit signature — calls `permit()` then swaps in one tx (no separate approve needed)
...
- **Security:** OZ `Ownable` for admin functions, OZ `ReentrancyGuard` (`nonReentrant`) on swap/transfer, OZ `Pausable` (`whenNotPaused`) for emergency stop, OZ `SafeERC20` for all token transfers, custom `onlyWhitelisted` modifier for token operations, owner/factory for whitelist management
...
### MockERC20.sol
- **Responsibility:** ERC20 token for testnet use — mintable by owner, gasless approvals
- **Tech:** Solidity 0.8.24, OpenZeppelin 5.x (ERC20, ERC20Burnable, ERC20Permit, Ownable)
- **Functions:** mint (onlyOwner), burn, burnFrom (inherited from ERC20Burnable), permit (EIP-2612 gasless approvals)

### MockDEX.sol
- **Responsibility:** Uniswap V2 style AMM — constant product pools for demo
- **Tech:** Solidity 0.8.24, Foundry, OpenZeppelin 5.x (SafeERC20)
...
- **Tech:** Solidity 0.8.24, Foundry, OpenZeppelin 5.x (AccessControl)
- **Roles:** `DEFAULT_ADMIN_ROLE` (manages roles), `CREATOR_ROLE` (can create tokens)
...
| Contracts | Solidity 0.8.24 + Foundry (nightly) + OpenZeppelin 5.x | Polkadot Hub compatible, battle-tested security primitives |
