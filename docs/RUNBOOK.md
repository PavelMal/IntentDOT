...
# Frontend tests (185 tests: intent-validator, risk-guardian, preview-builder, risk-display, xcm-encoder, bridge-flow, integration-flow, e2e-testnet)
cd frontend
npm test
...
> - **Solidity 0.8.24** with `evm_version = "london"` — avoids PUSH0 (EIP-3855) opcode generation
...
## Troubleshooting: Styles broken (no CSS)

**Symptom:** Page loads but without any styles — plain HTML, no colors, no layout.
**Cause:** `.next/` cache is corrupted (e.g. after a `next build` while dev server was running, or a crash).
**Fix:**
```bash
